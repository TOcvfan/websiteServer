const nytPassword = async (req, res, knexDb, bcrypt, dotenv, jwt) => {
    ...
    if (req.headers && req.headers.authorization) {
        ...
        try {
            ...
            return res.status(401).send({ message: 'unauthorized ' + e, error: true });
        }
    ...
    knexDb(tabel)
        .where('email', email)
        .then(data => {
            data[0] === undefined ? res.status(409).send({ message: 'findes ikke', error: true }) : sammenlignPassword()
        })
        .catch(err => res.status(400).json({ message: err + email, error: true }));
}
