const handleSignin = (req, res, knexDb, bcrypt, jwt, dotenv) => {
		...
		return knexDb(tabel).leftOuterJoin(profil, `${tabel}.id`, `${profil}.bruger_id`)
			...
			.then(user => {
				...
				knexDb(profil).where('id', '=', payload.id).update({ sidstOnline: new Date() })
					...
					}).catch(err => response('der skete en fejl ' + err, true, 400))
			})
			.catch(err => response('unable to get user ' + err, true, 400))
	}

	const sammenlignPassword = (pass) => {
		bcrypt.compareSync(password, pass) === true ?
			loadUser() : res.status(409).send({ name: 'password', message: 'Forkert kode', error: true })
	}
	...
	knexDb.select('brugernavn', 'email', 'userpassword').from(tabel)
		...
		.then(data => {
			data[0] === undefined ? res.status(409).send({ name: 'bruger', message: brugernavn, error: true }) : sammenlignPassword(data[0].userpassword)
		})
		.catch(err => response(err, true, 400));
}
